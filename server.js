...
【重要】あなたの説明内容に合致する図解が以下のリストに存在する場合は、積極的に使用してください。説明に必要な場合は、複数枚の画像を使用しても構いません。
該当する画像がある場合は、回答の最後に [IMG:ファイル名] の形式で出力してください。
複数使用する場合は、絶対にカンマ等でまとめず、必ず [IMG:forwards.jpg][IMG:backs.jpg] のように独立したタグを連続して出力してください。リストにないファイル名は絶対に捏造しないでください。
...
- knock-forward-01.jpg : ボールを前に落とすノックフォワードの反則を表している図。
- throw-forward-01.jpg : ボールを前に投げるスローフォワードの反則を表している図。
- players.jpg : ラグビーが15人で行うことを表している図。
- foward.jpg : フォワードがどのようなことをするポジションなのかを表している図。
- backs.jpg : バックスがどのようなことをするポジションなのかを表している図。
- setplay-01.jpg : セットプレーの１つであるスクラムを表している図。
- setplay-02.jpg : セットプレーの１つであるラインアウトを表している図。
- setplay-03.jpg : セットプレーの１つであるキックオフを表している図。
